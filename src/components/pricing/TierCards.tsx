  ...
  return (
    ...
    >
      ...
      <div className="relative mt-6">
        ...
        >
          {tier.price}
          <span className={`ml-0.5 text-[16px] tracking-[0px] font-semibold ${featured ? "text-white/60" : "text-muted"}`}>
            /project
          </span>
        </p>
      </div>
      ...
      <Link
        href="/contact"
        className={`relative mt-6 block rounded-full py-3.5 text-center text-[15px] font-bold transition-colors duration-300 ${
          featured
        ...
        {allFeatures.map((f) => {
          ...
          return (
            <li key={f} className="flex items-start gap-2.5 text-[14px] leading-snug">
              {on ? (
                <Check
                  className={`mt-px h-4 w-4 shrink-0 ${featured ? "text-[#7C3AED]" : "text-[#7C3AED]"}`}
                  strokeWidth={2.5}
                />
              ) : (
                <Minus
